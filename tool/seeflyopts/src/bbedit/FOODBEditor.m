...
@implementation FOODBEditor

static FOODBEditor	*_sharedODBEditor;

+ (id)sharedODBEditor
{
	if (_sharedODBEditor == nil)
	{
		_sharedODBEditor = [[FOODBEditor alloc] init];
	}
...
{
    // FIXME if we created a temporary file for this session should we try to delete it and/or close it in the editor?
...
- (void) removeODBClient:(id) client {
	...
	if (_filesBeingEdited) {
		
		NSArray *fileNames = [_filesBeingEdited allKeysForObject:client];
...
@implementation FOODBEditor(Private)
