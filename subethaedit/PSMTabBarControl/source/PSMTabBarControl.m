...
- (void)addTabViewItem:(NSTabViewItem *)item
{
	...
	NSRect cellRect = [self genericCellRect], lastCellFrame = [[_cells lastObject] frame];
    ...
    if([_cells count] == [tabView numberOfTabViewItems]){
        [self update:_automaticallyAnimates]; // don't update unless all are accounted for!
    }
}
