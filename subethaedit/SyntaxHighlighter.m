...
- (void)stateMachineOnAttributedString:(NSMutableAttributedString *)aString inRange:(NSRange)aRange {
    ...
    while (range.length>0) {
        NSRange foundRange;
        if (!state) {
            //foundRange=[[aString string] rangeOfRegex:@"(//|@\"|\"|/\\*)" options:0 range:range];
            if (foundRange.location!=NSNotFound) {
                NSString *foundString=[[aString string] substringWithRange:foundRange];
                if ([foundString isEqualTo:@"@\""] || [foundString isEqualTo:@"\""]) {
                    state=@"String";
                } else if ([foundString isEqualTo:@"/*"]) { // */
                    state=@"MultiComment";
                } else {
                    state=@"Comment";
                }
            ...
            //foundRange=[[aString string] rangeOfRegex:@"\n" options:nil range:range];
            if (foundRange.location!=NSNotFound) {
                range.length-=NSMaxRange(foundRange)-range.location;
                [aString addAttribute:@"StateMachineState" value:state range:NSMakeRange(stateStart,foundRange.location-stateStart)];
                state=nil;
        ...
        } else if ([state isEqualToString:@"MultiComment"]) {
            //foundRange=[[aString string] rangeOfRegex:@"\\*/" options:nil range:range];
            if (foundRange.location!=NSNotFound) {
                range.length-=NSMaxRange(foundRange)-range.location;
                [aString addAttribute:@"StateMachineState" value:state range:NSMakeRange(stateStart,NSMaxRange(foundRange)-stateStart)];
                state=nil;
                [aString addAttribute:NSForegroundColorAttributeName value:[NSColor greenColor] range:NSMakeRange(stateStart,NSMaxRange(foundRange)-stateStart)];
                stateStart=foundRange.location;
                range.location=NSMaxRange(foundRange);
        ...
        } else if ([state isEqualToString:@"String"]) { 
            //foundRange=[[aString string] rangeOfRegex:@"[^\\\\](\\\")|(^\\\")" options:nil range:range];
            if (foundRange.location!=NSNotFound) {
                state=nil;
                range.length-=NSMaxRange(foundRange)-range.location;
                [aString addAttribute:@"StateMachineState" value:@"String" range:NSMakeRange(stateStart,NSMaxRange(foundRange)-stateStart)];
                [aString addAttribute:NSForegroundColorAttributeName value:[NSColor redColor] range:NSMakeRange(stateStart,NSMaxRange(foundRange)-stateStart)];
                stateStart=foundRange.location;
                range.location=NSMaxRange(foundRange);
