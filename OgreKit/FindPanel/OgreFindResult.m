...
static const unsigned   OgreFindResultInitialCapacity = 30;

@implementation OgreFindResult

- (id)initWithString:(NSString*)text syntax:(OgreSyntax)syntax color:(NSColor*)highlightColor
{
#ifdef DEBUG_OGRE_FIND_PANEL
	NSLog(@"-initWithString: of OgreFindResult");
#endif	
	self = [super init];
	if (self) {
		/* 1行目の範囲を得る */
		...
		_lineOfMatchedStrings = [[NSMutableArray alloc] initWithCapacity:OgreFindResultInitialCapacity];
		[_lineOfMatchedStrings addObject:[NSNumber numberWithUnsignedInt:0]];
		_matchRangeArray = [[NSMutableArray alloc] initWithCapacity:OgreFindResultInitialCapacity];
...
{
#ifdef DEBUG_OGRE_FIND_PANEL
	NSLog(@"-dealloc of OgreFindResult");
#endif
...
{
#ifdef DEBUG_OGRE_FIND_PANEL
	NSLog(@"-windowWillClose: of OgreFindResult");
#endif
	[[NSNotificationCenter defaultCenter] removeObserver:self];
	_targetToFindIn = nil;
}
...
{
#ifdef DEBUG_OGRE_FIND_PANEL
	NSLog(@"-updateOldRange: of OgreFindResult");
#endif
	// Notation
	//  (a b) : changed range
	...
	[(id <OgreFindResultDelegateProtocol>)_delegate didUpdateTextFindResult:self];
}
...
{
#ifdef DEBUG_OGRE_FIND_PANEL
	NSLog(@"-updateSubranges: of OgreFindResult");
#endif
